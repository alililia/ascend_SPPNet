...
├── cv
    ├── sppnet
        ...
        │   ├──run_distribution_.sh             // 多卡训练+推理的shell脚本
        │   ├──run_standalone_eval_.sh          // 单卡推理的shell脚本
        │   ├──run_standalone_train_.sh         // 单卡训练+推理的shell脚本
        ├── src
        │   ├──dataset.py                             // 创建数据集
...
- 处理器环境运行
  ...
  ```shell
  # 单卡训练zfnet
  python train.py --train_path ./imagenet/train --device_target  --eval_path ./imagenet/val --device_id 0 --train_model zfnet > log 2>&1 &

  # 或进入脚本目录，执行脚本
  bash run_standalone_train_.sh ./imagenet_original/train ./imagenet_original/val 0 zfnet

  # 分布式训练zfnet，进入脚本目录，执行脚本
  bash run_distribution_.sh ./imagenet_original/train ./imagenet_original/val zfnet

  # 单卡训练sppnet(single train)
  python train.py --train_path ./imagenet/train --device_target  --eval_path ./imagenet/val --device_id 0 > log 2>&1 &

  # 或进入脚本目录，执行脚本
  bash run_standalone_train_.sh ./imagenet_original/train ./imagenet_original/val 0 sppnet_single

  # 分布式训练sppnet(single train)，进入脚本目录，执行脚本
  bash run_distribution_.sh  ./imagenet_original/train ./imagenet_original/val sppnet_single

  # 单卡训练sppnet(mult train)
  python train.py --train_path ./imagenet/train --eval_path ./imagenet/val --device_target  --device_id 0 --train_model sppnet_mult > log 2>&1 &

  # 或进入脚本目录，执行脚本
  bash run_standalone_train_.sh ./imagenet_original/train ./imagenet_original/val 0 sppnet_mult

  # 分布式训练sppnet(mult train)，进入脚本目录，执行脚本
  bash run_distribution_.sh ./imagenet_original/train ./imagenet_original/val sppnet_mult
  ```
...
- 处理器环境运行
  ...
  ```shell
  python eval.py --data_path ./imagenet_original/val --ckpt_path ./ckpt/best.ckpt --device_target  --device_id 0 --train_model sppnet_single > eval_log.txt 2>&1 &

  # 或进入脚本目录，执行脚本

  sh run_standalone_eval_.sh ./imagenet_original/val ../ckpt/best.ckpt 0 sppnet_single
  ```
